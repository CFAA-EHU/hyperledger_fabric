...
def invoke_chaincode(timestamp, user, stage, fhash):
    ...
    command = [
        ...
        '-c', f'{{"function":"CreateRecord","Args":["{timestamp}","{user}","{stage}","{fhash}"]}}'
...
class CSVHandler(FileSystemEventHandler):
    def on_created(self, event):
        ...
        if not event.is_directory and event.src_path.endswith(".csv"):
            ...
            directorio_base = "/home/ubuntu/fabric-samples/red-propia/FRESADO/"

            # Obtener la ruta relativa desde el directorio base
            ruta_relativa = os.path.relpath(event.src_path, directorio_base)
            print(ruta_relativa)

            partesruta = ruta_relativa.split('/')
            usuario = partesruta[0]
            etapa = partesruta[1]
            ...
            invoke_chaincode(timestamp_ns, usuario, etapa, hash)
...
if __name__ == "__main__":
    ...
    ruta_base = "/home/ubuntu/fabric-samples/red-propia/FRESADO"  # Cambia esto a la ruta de tu sistema
