...
func (s *SmartContract) InitLedger(ctx contractapi.TransactionContextInterface) error {
	records := []Record{
		{Timestamp: "1726643095000000000", User: "ENDIKA", Stage: "PROGRAMA", FileHash: "hash1"},
	}
...
func (s *SmartContract) CreateRecord(ctx contractapi.TransactionContextInterface, timestamp string, user string, stage string, fileHash string) error {
...
func (s *SmartContract) UpdateRecord(ctx contractapi.TransactionContextInterface, timestamp string, user string, stage string, fileHash string) error {
	exists, err := s.RecordExists(ctx, timestamp)
	if err != nil {
		return err
	}
